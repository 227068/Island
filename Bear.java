...
public class Bear extends Carnivore {
    ...
    public String getName() {
        return "\uD83D\uDC3B Медведь"; // Bear emoji and name in Russian
    }
    ...
    public void eat(Location location) {
        ...
        if (!herbivores.isEmpty() && random.nextInt(100) < 80) {
    ...
    public void reproduce(Location location) {
        // Count the number of bears in the location
        long bearCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Bear)
                .count();

        // Check reproduction conditions
        if (bearCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Bear());
            System.out.println("Родился новый медведь! \uD83D\uDC3B"); // New bear is born!
        }
    }
