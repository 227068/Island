...
public class Buffalo extends Herbivore {
    ...
    public String getName() {
        return "\uD83D\uDC03 Буйвол"; // Buffalo emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of buffalo in the location
        long buffaloCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Buffalo)
                .count();

        // Check reproduction conditions
        if (buffaloCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Buffalo());
            System.out.println("Родился новый буйвол! \uD83D\uDC03"); // New buffalo is born!
        }
    }
