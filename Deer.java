...
public class Deer extends Herbivore {
    ...
    public String getName() {
        return "\uD83E\uDD8C Олень"; // Deer emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of deer in the location
        long deerCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Deer)
                .count();

        // Check reproduction conditions
        if (deerCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Deer());
            System.out.println("Родился новый олень! \uD83E\uDD8C"); // New deer is born!
        }
    }
