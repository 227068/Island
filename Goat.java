...
public class Goat extends Herbivore {
    ...
    public String getName() {
        return "\uD83D\uDC10 Коза"; // Goat emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of goats in the location
        long goatCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Goat)
                .count();

        // Check reproduction conditions
        if (goatCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Goat());
            System.out.println("Родилась новая коза! \uD83D\uDC10"); // New goat is born!
        }
    }
