...
public class Horse extends Herbivore {
    ...
    public String getName() {
        return "\uD83D\uDC0E Лошадь"; // Horse emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of horses in the location
        long horseCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Horse)
                .count();

        // Check reproduction conditions
        if (horseCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Horse());
            System.out.println("Родилась новая лошадь! \uD83D\uDC0E"); // New horse is born!
        }
    }
