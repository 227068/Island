...
public class Mouse extends Herbivore {
    ...
    public String getName() {
        return "\uD83D\uDC01 Мышь"; // Mouse emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of mice in the location
        long mouseCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Mouse)
                .count();

        // Check reproduction conditions
        if (mouseCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Mouse());
            System.out.println("Родилась новая мышь! \uD83D\uDC01"); // New mouse is born!
        }
    }
