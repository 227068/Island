...
public class Sheep extends Herbivore {
    ...
    public String getName() {
        return "\uD83D\uDC11 Овца"; // Sheep emoji and name in Russian
    }
    ...
    public void reproduce(Location location) {
        // Count the number of sheep in the location
        long sheepCount = location.getAnimals().stream()
                .filter(animal -> animal instanceof Sheep)
                .count();

        // Check reproduction conditions
        if (sheepCount >= 2 && location.getAnimals().size() < Config.MAX_ANIMALS_PER_LOCATION && random.nextInt(100) < Config.REPRODUCTION_CHANCE) {
            location.getAnimals().add(new Sheep());
            System.out.println("Родилась новая овца! \uD83D\uDC11"); // New sheep is born!
        }
    }
